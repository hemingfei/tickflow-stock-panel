...
async def _application_lifespan(app: FastAPI):
    ...
    depth_svc = DepthService()
    depth_svc.set_repo(repo)
    depth_svc.set_app_state(app.state)
    app.state.depth_service = depth_svc
    ...
    try:
        ...
        intraday_sentiment_service.set_depth_service(depth_svc)
    ...
    try:
        ...
        if wbot:
            wbot.stop()
        issvc = getattr(app.state, "intraday_sentiment_service", None)
        if issvc:
            issvc.stop()
...
if _static.exists():
    ...
    def spa_fallback(full_path: str):  # noqa: ARG001
        ...
        index.html 禁止缓存 (Cache-Control: no-store), 保证浏览器每次拿到
