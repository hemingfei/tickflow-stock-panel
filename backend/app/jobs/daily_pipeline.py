...
def _maybe_push_review(content: str, meta: dict) -> None:
    ...
    channels 为空则不推送; 'feishu'/'kol'/'custom'/'email' 复用监控中心的全局外部渠道配置。
    ...
    try:
        ...
        for ch in channels:
            if ch == "feishu":
                ...
                if not url:
                    logger.info("review push(kol) skipped: webhook not configured")
                    continue
                # KOL 简化格式: msg_id 按报告日幂等 (同日重复推送不重复发帖)
                msg_id = f"tickflow-review-{as_of or cn_now().strftime('%Y-%m-%d')}"
                ok = webhook_adapter.send_kol(
                    url, "每日复盘", f"{subtitle}\n\n{content}".strip(),
                    preferences.get_kol_webhook_secret(), msg_id=msg_id,
                )
                logger.info("review push(kol) %s", "sent" if ok else "failed")
            elif ch == "custom":
