...
// ===== Watchlist Groups (自选板块 — hmf 分支独立页面) =====
// 与 main 的 WatchlistGroup (id/name/color, M:N 分组) 是两套并存的数据结构
export interface WatchlistGroupBoard {
  group_id: string
  name: string
  order: number
...
export interface MonitorRule {
  ...
  webhook_channels?: string[]  // 命中时推送的外部渠道 (合法值 'feishu' | 'wecom' | 'kol' | 'custom' | 'email')
  created_at?: string
...
export const api = {  health: () => request<{ status: string; version: string; mode: string }>('/health'),
  ...
  sendTestWebhook: (channel: 'feishu' | 'wecom' | 'kol' | 'custom' | 'email') =>
  ...
  // ── 自选板块 (hmf 分支独立页面 WatchlistGroups, 后端 /api/watchlist-groups/*) ──
  // 命名避开 main 的扁平 watchlistGroups()/watchlistGroupCreate 系列方法
  watchlistGroupBoards: {
    list: () =>
      request<{ groups: WatchlistGroupBoard[] }>('/api/watchlist-groups'),
    ...
    rename: (groupId: string, name: string) =>
      request<{ groups: WatchlistGroupBoard[] }>(`/api/watchlist-groups/${encodeURIComponent(groupId)}`, {
        method: 'PATCH',
        body: JSON.stringify({ name }),
      }),
    ...
    reorder: (groupIds: string[]) =>
      request<{ groups: WatchlistGroupBoard[] }>('/api/watchlist-groups/order', {
        method: 'POST',
    ...
    importConfig: (data: any, replace: boolean = false) =>
      request<{ groups: WatchlistGroupBoard[] }>('/api/watchlist-groups/import', {
        method: 'POST',
